  ...
  <div class="content bg-color py-3">
      <div class="container">
          <div class="card card-body border-0">
              <h3 class="font-24 bold">Post a Ads</h3>
            ...
            <form>
                  ...
                  <div class="mb-3">
                   ...
                   <small v-if="stepErrors.title" class="text-danger">{{stepErrors.title}}</small>
                    ...
                    <div class="section">
                        ...
                        <div class="item">
                            <input type="checkbox" v-model="form.any" true-value="1" false-value="0" class="btn-check" id="Any">
                ...
                <div v-if="StepThreeShow" class="step3">
                    ...
                    <div class="section">
                        ...
                        <div v-for="(preview, index) in featureImagePreview" :key="index" class="feature">
                            <img :src="preview" class="fullImage">
                            <div class="feature-action">
                                <button class="cancle" type="button" @click="removeFeatureImage(index)">
                                    <img src="/images/Post-Ad-Step-1/trash.png" width="16">
                                    <span class="ps-2">Remove</span>
                                </button>
                            </div>
                        </div>

                        <div v-if="featureImagePreview.length < 4" class="feature" :class="{'error' : stepErrors.featureImage}">
                            <img src="/images/Post-Ad-Step-1/defaultFeture.png" class="normalImage">
                            <input type="file" id="feature2" style="display: none;" accept="image/*" @change="displayFeature($event)">
                            <div class="feature-action">
                                <label for="feature2" class="upload">
                    ...
                    <small class="text-danger">{{ stepErrors.featureImage }}</small>
                    ...
                    <div class="mt-5 mb-2 d-flex justify-content-between">
                        ...
                        <button class="btn btn-primary px-4" type="button" @click="storePost()"> Finish</button>
                    </div>
                </div>
...
export default {
  ...
  watch: {
    ...
    'form.any': {
      handler: function(v) {
      this.form.male_student = 0
      this.form.women_job = 0
      this.form.man_job = 0
      this.form.female_student = 0
    }
    },
  ...
  methods: {
    ...
    displayFeature(event){
      ...
      this.featureImagePreview.push(preview);
      const File = event.target.files[0];
      this.feature_image.push(File)
    },
    removeFeatureImage(index){
      this.feature_image.splice(this.feature_image.indexOf(index),1)
      this.featureImagePreview.splice(index,1);
    },
  ...
  storePost(){
    this.stepErrors.featureImage = !this.feature_image.length ? 'The feature image field is required!' : '';
    if (!this.feature_image.length) {
      return false
    }
    ...
    for( var i = 0; i < this.feature_image.length; i++ ){
      formData.append('feature_image[' + i + ']', this.feature_image[i]);
    }

    this.$store.commit("loader/SET_Loader",true)

    this.$store.dispatch("rent/createNewPost",formData).then( res => {
      this.$store.commit("loader/SET_Loader",false)
      this.$toast.success(res.message)
      this.$router.push('/finish-post')
    })
    .catch(err => {
      this.$store.commit("loader/SET_Loader",false)
